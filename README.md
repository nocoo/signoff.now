...
- **Background collection**: Project discovery fully refreshes accessible PR lists and states every 10 minutes after completion. Each watched PR independently collects state, checks, builds and stages, then cools down for 5 minutes. Both intervals are configured in Collector details; manual requests also respect cooldown. The dialog groups attempt history, phases, returned snapshots and next-run countdowns by PR/project. Collection continues when the webpage closes.

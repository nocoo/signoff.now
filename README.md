...
- **跨项目 PR 队列**：按 Organization → Project → Repository 筛选，默认排除 Draft，作者可多选。筛选保存在 URL 和 localStorage，每页 20 个 PR。
...
- **PR 阅读与操作**：描述支持 Markdown、表格和任务清单；真实 PR 标题旁可直接打开源页面。ADO 的 Proof Of Presence 简称 PoP，待人工认证时显示黄色，只有其他必需条件全部通过后才成为主 Readiness。

...
Non-main targets (`main`/`master`, optional `refs/heads/` prefix, case-sensitive) are `skipped`/Skipped, cyan, excluded from Jev even when watched. This is a branch-name rule, not a provider default-branch lookup. Skipped has `status=complete`, `current=null`, `previous=null`, and takes precedence over conflict. Watched main-target PRs use `conflict`, `attention`, `warning`, `running`, `ready`, `waiting`. Conflict is determined from provider mergeability and excluded from Jev. Other business categories come from one Jev Choice per PR. Build failure means Attention; SignOff does not choose rerun versus repair. Waiting is only for external reviewers after successful unexpired builds; PoP is last. Ready includes the configured PoP-only final-step convention and never overrides provider merge requirements. `unknown`/`error` describe operational inference state. Read `readiness.status` and `current`; pending/evaluating/error may include historical `previous`. Unwatched main-target PRs are `not_watched`. There is no `readiness.ready`, `on_track`, first-blocker authority or action Choice. API status filters/counts include `skipped`; additional API sort fields are `repository`, `author`, `target`, `evaluated`, `stateChecked`, and `checksChecked`. Queries never trigger inference. Only visible focused dashboard ticks initiate changed watched PR batches, independently by project, default 5-minute completion cooldown in Connector details. Background time counts. Configure the encrypted key and common/project rules in AI Settings. Fixed shared policy codes and scoped explanations appear in State machines. See `docs/19-pr-state-machines.md`.

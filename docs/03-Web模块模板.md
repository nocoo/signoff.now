...
signoff 的 Web 不是从零发明，而是**显式嫁接**两条已验证路径：

| 参考仓库 | 路径（相对 monorepo 根） | 借什么 |
|:---------|:--------------------------|:-------|
| **basalt** | `../basalt` | 设计语言、Vite/React/Tailwind/shadcn、**MVVM 目录约定**、Model/ViewModel **≥95%** 覆盖、Husky L1+G1+G2 形态 |
| **bat** | `../bat` | CF Worker 双入口、**Access JWT + JWKS**、`entry-control` / `access-auth` / `api-key` 中间件链、六维质量门、L2 本地 Wrangler E2E |
...
| pre-commit / pre-push | `.husky/*` + `osv-scanner.toml` | 对齐 monorepo 根 hooks（见 §6） |
| 主题 | `ThemeToggle` system→light→dark | 一期保留；与 basalt 三态一致 |

basalt **没有** CF Access / D1；登录页（`/login`、`/badge-login`）仅作视觉参考，**生产鉴权走 Access 边缘拦截**，不自建账号密码。

### 2.2 bat：Worker 与鉴权

| 资产 | bat 位置 | signoff 用法 |
|:-----|:---------|:-------------|
| 双入口 | `entry-control.ts` | Browser host vs ingest host 分流 |
| Access JWT | `access-auth.ts`（`jose` + remote JWKS） | 浏览器 API 必验；失败 401/403；未配置 fail-closed 500 |
| API Key | `api-key.ts` | 映射为 **Pipeline Token**（ingest / recompute）与可选 machine-read |
| `/api/me` | `routes/me.ts` | 侧栏展示邮箱；无 JWT 时 `authenticated: false`（本地 dev） |
| SPA 静态 | Worker `[assets]` + UI build 输出到 worker static | 同 Worker 托管 SPA + API |
| 质量六维 | `docs/07-testing.md` | L1/L2/G1/G2 映射到 signoff（见 §5–§6） |
| 本地绕过 | `isLocalhost` + `*.dev.hexly.ai` | 本地 / Caddy dev 跳过 Access，便于 E2E |

bat UI 层较薄（SWR + routes，非严格 MVVM）；**前端分层以 basalt 为准**，**鉴权与部署以 bat 为准**。
...
| bat 的 probe / Rust / Netdata 替换语义 | 产品无关 |
...
apps/worker/  或  packages/worker/ # Hono Worker（bat 形态）— 实现时二选一，推荐 packages/worker
└── src/
    ├── index.ts                   # middleware 链 + routes
    ├── middleware/
    │   ├── entry-control.ts
    │   ├── access-auth.ts
    │   └── pipeline-auth.ts       # 对应 bat api-key，名随 signoff 语义
    └── routes/
        ├── me.ts
        ├── live.ts
        ├── settings.ts            # 04
...
- **SWR 或 TanStack Query** 二选一（bat 用 SWR；任选其一写进实现，不要混用）  
...
bat 已声明 design system = Basalt。signoff 与 bat **共享同一套 token 语义**，视觉家族一致；业务组件不互相 import 仓库路径，**拷贝 token + 组件源**进 `apps/web`。
...
| **L3** | Playwright（可选，二期） | 登录后主路径 | CI only（对齐 bat） |

### 6.2 vitest 配置模板（对齐 basalt + 当前 scaffold）
...
- 禁止真实网络；`fetch` mock  
- Worker Access：fixture JWT + mock JWKS（见 bat `access-auth.test.ts` 模式）  
...
### 7.1 六维映射（bat `07-testing` → signoff）
...
| pre-push | Worker L2 E2E；可选 `gate:routes`（OpenAPI/路由表与测试对齐） |
| CI | 全量 L1 + G1 + G2 + L2；tag 发版再 L3 |
| 工具缺失 | bat 式 `scripts/ensure-tools.sh`：无 `gitleaks`/`osv-scanner` 则 fail 并提示安装 |

### 7.4 Biome / 安全配置约定

- **Biome 0 warnings** 门禁（与 basalt/bat 一致）  
- `osv-scanner.toml`：仅允许**有理由**的间接 dev 依赖 ignore（模板抄 basalt，禁止无理由全关）  
- `.data/` 采集落盘 **gitignore**；gitleaks 防 token 入库  
- 覆盖率不过 → commit 失败；漏洞不过 → push 失败

### 7.5 basalt 单仓 hooks 对照（前端包级）

basalt pre-commit 更重：`typecheck + lint + test + gitleaks + build + test:coverage`；pre-push：`osv-scanner`。  
...
参考实现：`../bat/packages/worker/src/middleware/access-auth.ts`（可几乎原文件移植，改 env 名与 host 判断）。

### 8.4 `/api/me`

- JWT 已由 `accessAuth` 验证（生产 browser）。  
- Payload 解码取 `email` / `name`（bat `decodeJwtPayload`：base64url payload，**不**在 me 路由再验签——验签只在 middleware 一次）。  
- 响应：
...
| 方式 | 行为 |
|:-----|:-----|
| `bun run dev`（Vite only） | API 可 proxy 到 `wrangler dev`；无 JWT，`/api/me` 匿名 |
| Caddy `*.dev.hexly.ai` | 与 bat 一致纳入 `isLocalhost` 族，跳过 Access |
| 生产联调 | 真实 Access；用 allowlist 测试账号 |

前端**禁止**实现「输入邮箱密码」的假登录作为生产路径；basalt 登录页仅 demo。
...
| PR | 内容 | 验收 |
|:---|:-----|:-----|
| W1 | 拷 basalt tokens + DashboardLayout + 空路由壳 | 视觉 L0/L1；无业务 |
| W2 | `models/` + `viewmodels/` 目录约定 + vitest include 收窄 | coverage 门绿 |
| W3 | Worker 骨架：entryControl + accessAuth + pipelineAuth + `/api/live` + `/api/me` | 单测 + 本地 host 矩阵 |
...
| **03（本文）** | Web/Worker **模板**：设计系统、MVVM、测试、hooks、Access |
| [04](./04-Settings设计.md) | Settings 专篇（在本模板上落第一页） |
...
| 路径 | 用途 |
|:-----|:-----|
| `../basalt` | UI / MVVM / coverage / tokens |
| `../bat/packages/worker/src/middleware/*` | Access 与双入口 |
| `../bat/docs/06-ui.md`、`07-testing.md`、`02-architecture.md` | 叙事对齐 |

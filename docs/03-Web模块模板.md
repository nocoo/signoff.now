...
> 状态：Basalt / MVVM 模板已采用；本文保留早期落地计划。当前功能见 [11](11-真实PR采集与本地工作台.md)、[13](13-成员目录与PR贡献统计.md)，各项测试门禁的实际执行与差距以 [AGENTS.md](../AGENTS.md) 为准。
...
| 本地绕过 | `isLocalhost`：`localhost` / `127.0.0.1` / `*.dev.hexly.ai` | 与 bat 相同；Caddy / proxy 本地跳过 Access 与 Pipeline Token |
...
> 鉴权模型与中间件链 **对齐 bat**（`../bat/packages/worker/src/middleware/*`、`../bat/docs/19-edge-deployment.md`），仅改名与路由表适配 signoff。不另造 ENVIRONMENT 双因子或更重的鉴权框架。

### 8.1 鉴权范围（对照 bat 三 scope）

| Scope | 机制 | 谁用 | 典型路由 |
|:------|:-----|:-----|:---------|
| **Pipeline write** | `Authorization: Bearer <PIPELINE_WRITE_TOKEN>` | CLI / 本机采集 | `POST /api/pipeline/ingest`、`POST /api/pipeline/recompute/*` |
| **Pipeline read** | 独立 read token，或一期与 write 同 secret 再收窄 | CLI bootstrap | `GET /api/pipeline/bootstrap` |
| **Browser** | Cloudflare Access JWT | 管理者 SPA | `GET/PUT /api/settings`、Developers/Repos CRUD、`GET /api/me` 等 |
| **Public** | 无 | 健康检查 | `GET /api/live` |

同 Worker 进程，靠 **Host** 分流（`entry-control`），避免「只靠前端藏路由」。

| 入口 | 域名（示例） | 边缘 | Worker 内 |
|:-----|:-------------|:-----|:----------|
| **Browser** | `signoff.example.com` | Access 应用（IdP + email allowlist） | `accessAuth` 验 JWT |
| **Machine / Pipeline** | `signoff-ingest.example.com`（或机器专用 host） | **无** Access | 白名单 path + Pipeline Token |
| **Local** | 见 §8.2 `isLocalhost` | 无 | 跳过 Access / Pipeline Token（本地 dev / E2E） |

### 8.2 中间件顺序（必须；对照 bat）

```
request
  → entryControl      // host → localhost | machine whitelist | browser pass-through
  → accessAuth        // browser：验 JWT；machine / localhost：skip
  → pipelineAuth      // 对照 bat apiKeyAuth：token 或 Access 已通过则放行
  → handlers
```

#### `entryControl`（对照 bat `entry-control.ts`）

| Hostname 模式 | 行为 |
|:--------------|:-----|
| `localhost` / `127.0.0.1` / `*.dev.hexly.ai` | **Bypass** entry 限制 → 继续后续中间件（本地 dev / E2E） |
| `signoff-ingest.*`（machine） | **仅白名单** method+path；其它 → **403** |
| 其余（browser，如 `signoff.example.com`） | Pass through → `accessAuth` |

Machine 白名单（signoff 一期建议）：

| Method | Path | 用途 |
|:-------|:-----|:-----|
| GET | `/api/pipeline/bootstrap` | CLI 拉 Settings/实体 |
| POST | `/api/pipeline/ingest` | 写 Activity/Score |
| POST | `/api/pipeline/recompute/complete` | 清 stale |
| GET | `/api/live` | 健康检查 |
| GET | `/api/me` | 与 bat 一致，便于探测 |

未列出的管理 API（如 `PUT /api/settings`）在 machine host 上 **403**——防止 ingest 域名被用来改管理配置。

#### `isLocalhost`（对照 bat，原样语义）

```ts
// 与 bat entry-control 一致：不要额外要求 ENVIRONMENT 标志
export function isLocalhost(host: string): boolean {
  return (
    host.startsWith("localhost") ||
    host.startsWith("127.0.0.1") ||
    host.endsWith(".dev.hexly.ai") // Caddy 本机 dev
  );
}
```
...
- 生产 browser / ingest 域名**不会**命中上述模式；本地 bypass 仅服务 dev/E2E。  
- Vite `proxy` 的 `target` 为 `http://127.0.0.1:37042` 时，Worker 侧 Host 已是 `127.0.0.1`，天然走 localhost 分支；`*.dev.hexly.ai` 覆盖浏览器直打 Caddy 域名的情况（与 bat 相同）。  
- **不**为「防伪造 Host」再叠一层 env 开关——与 bat 一致，靠生产域名不匹配 + Access fail-closed。

#### `accessAuth`（对照 bat `access-auth.ts`）

**Skip（直接 `next()`）：** `isLocalhost`、machine host、公开 `GET /api/live`。

**Browser host：**

1. 未配置 `CF_ACCESS_TEAM_DOMAIN` / `CF_ACCESS_AUD` → **500** fail-closed（禁止静默降级）。  
2. 读 `Cf-Access-Jwt-Assertion`；缺失 → **401**。  
3. `jwtVerify` + JWKS（`https://${teamDomain}/cdn-cgi/access/certs`，按 teamDomain 缓存）。  
4. 失败 → **403**；成功 → `c.set("accessAuthenticated", true)`。

**不得**仅凭「header 存在」信任 JWT；下游只看 context flag。
...
| `SIGNOFF_PIPELINE_WRITE_TOKEN`（或 `PIPELINE_WRITE_TOKEN`） | 采集写入 |
| `SIGNOFF_PIPELINE_READ_TOKEN`（可选；一期可与 write 同值） | bootstrap 只读 |

### 8.4 `/api/me`

- 生产 browser：JWT 已由 `accessAuth` 验证。  
...
### 8.5 Pipeline Token 与 CLI

| 用途 | 机制 | 路由示例 |
|:-----|:-----|:---------|
| 采集 ingest | Bearer write token | `POST /api/pipeline/ingest` |
| bootstrap 只读 | Bearer read（或 write）token | `GET /api/pipeline/bootstrap` |
| 浏览器 CRUD | **仅** Access，不用 pipeline token | `PUT /api/settings` 等 |

CLI 读 Settings：见 [04 §6](./04-Settings设计.md)——经 **已鉴权 HTTP** 拉 D1 同源配置；token 只放本机环境变量 / secret manager，永不进仓库。推荐 `SIGNOFF_API_BASE` 指向 machine 入口或本机 `http://127.0.0.1:37042`。

### 8.6 Cloudflare Dashboard 配置清单

- [ ] Zero Trust → Access → Application：覆盖 browser 域名（含 `/api/*` 与 SPA）  
- [ ] Identity provider + **email allowlist**（管理者集合）  
- [ ] 复制 Application **AUD** → Worker `CF_ACCESS_AUD`  
- [ ] Team domain → `CF_ACCESS_TEAM_DOMAIN`  
- [ ] **不要**给 ingest / machine 域名套同一 Access 应用（否则 CLI 无法非交互推数）  
...
| 方式 | 行为 |
|:-----|:-----|
| `bun run dev`（Vite only） | 端口 **7042**；`/api` proxy → `127.0.0.1:37042`；Worker Host 为 loopback → `isLocalhost`，无 JWT，`/api/me` 匿名 |
| Caddy **`https://signoff.dev.hexly.ai`** | TLS → `localhost:7042`；Host 后缀 `*.dev.hexly.ai` → 同 bat，跳过 Access / Pipeline Token |
...
| browser host + 有效 JWT + `PUT /api/settings` | 2xx（无需 Pipeline Token） |
| `localhost` / `127.0.0.1` / `*.dev.hexly.ai` + 无 JWT | 放行（dev/E2E） |
| machine host + 非白名单路径（如 `PUT /api/settings`） | 403 |
| machine host + 正确 Bearer + ingest/bootstrap | 2xx |
| machine host + 无 Bearer + pipeline 路由 | 401 |
...
- [ ] Access / 双入口：对照 bat（entryControl + accessAuth + pipelineAuth），JWKS 验签、fail-closed、context flag  
- [ ] machine host 白名单；browser Access 管理 API 无需 Pipeline Token  
- [ ] `isLocalhost` 与 bat 同语义（含 `*.dev.hexly.ai`），无额外 ENVIRONMENT 开关  
- [ ] `/api/me` 行为与本地匿名一致  
- [ ] Pipeline Token 不进 git；CLI 走 machine / loopback + Bearer  

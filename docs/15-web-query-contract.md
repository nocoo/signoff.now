...
同一响应中的记录、计数和 `dataRevision` 必须对应同一个 source 版本。仓库、详情与观察查询使用一个一致性 batch；PR 分页先读取 open PR 的 readiness 事实，再通过 SQL 读取当前页、计数和作者，并校验两次读取的版本一致，不返回混合版本。版本改变时，无游标读取最多重试两次；带游标读取返回 `SNAPSHOT_CHANGED`，见 18。网页的 PR 表格与待采集清单各自按页独立读取当时版本，切换来源或范围重置相应分页，移除末页最后一项后回到有效页。多个数据块允许短暂处于不同版本，不能据此报错或循环重载全页。
...
- 待采集清单拥有独立分页；其首屏错误仍显示该块，后续错误保留最后成功结果。重试只读取待采集清单，逐项移除仍携带 observation generation。

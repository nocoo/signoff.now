...
每条依赖 SQL 都重复项目 revision / 目标条件，任何冲突使该配置变更整体不生效。旧 claim 不能在项目变更后发布或重新激活观察。项目页已移除旧启停按钮。单纯移除观察或终态淘汰保留 PR 快照；用户删除项目本身仍遵循其已有的数据删除语义；任务 canceled 摘要和自足观察记录不会随项目外键级联删除。

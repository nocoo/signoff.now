...
- ADO build policy 的明确 `isExpired=true` 标记为过期失败；`isExpired=false` 保留未过期，缺失则不补造到期证据。`buildIsNotCurrent` 不代表过期：目标提交变化后，策略可在有效期内继续 approved。普通 queued / pending 与没有到期证据的 policy 不被推断为过期。Raw policy evidence displays Build Expired; Jev independently judges intervention from all evidence and policy instructions.

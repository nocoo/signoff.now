...
Repository, author, target branch, PR updated, state checked and checks collected are separate sortable columns. Target branches wrap in full. Checks/stages and next action use bounded widths; action text is capped at two lines with the full action on hover and in PR details. Horizontal scrolling stays inside the table on narrow screens. API `sort` additionally accepts `repository`, `author`, `target`, `stateChecked`, and `checksChecked` in either direction; timestamp sorting uses actual timestamps, not relative labels. Missing checks sort before collected checks in ascending order.

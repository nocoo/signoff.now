...
应用层在 CAS 通过后，用 **一次** `env.DB.batch([...])` 提交下列语句（示意；非独立 round-trip）：
...
CAS：先确认 `expectedVersion` 等于当前 `pipeline_config_version` 的 JSON number；冲突则 **不**执行 batch。

**管线成功清 stale（须 body `pipelineConfigVersion` 与当前相等，见 §6.7）：**
...
2. 读出旧值，计算 diff；若 `expectedVersion !==` 当前 `pipeline_config_version` → **409**（不写库）。  
3. 用 **一次** `env.DB.batch([...])` 提交（不要先读后写再分次 `prepare().run()` 当「事务」）：  
   - 更新变化的业务键 `timezone` / `email_suffixes` / `activity_weights`  
   - 若任一「影响计分/匹配」的字段变化（见矩阵）→ 同 batch 内：  
     - `pipeline_config_version = expectedVersion + 1`（或 SQL 侧 `CAST(value AS INTEGER) + 1`，但必须以 CAS 成功为前提）  
     - `scores_stale = true`  
     - `scores_stale_reason = "<human readable>"`  
4. **不**在 Web 请求内跑 az / rematch（本机权限）；只标记状态。  
5. 返回新 `AppSettings` + 标志：
...
| 401/403 | Access |
| 409 | **`expectedVersion` 与当前 `pipeline_config_version` 不等**（并发写 / 过期表单） |
| 500 | D1 / JSON 约束失败 |
...
- 相等 → 接受写入  
- 不等（更旧 **或** 更新）→ **409**，CLI 须 `settings pull` 后重跑  

「未来版本」不得被接受（防时钟/缓存错乱写入）。
...
服务端：仅当 `pipelineConfigVersion ===` 当前 `pipeline_config_version` 时，才将 `scores_stale=false` 且 `scores_stale_reason=null`；否则 **409**（旧 rematch 任务不得清掉新配置触发的 stale）。  
也可在 **同一** full rematch 成功的 ingest 事务内、版本仍相等时由服务端清除——同样做相等判断，无需 run lease。
...
| API | PUT suffixes → version+1 且 stale；PUT 相同值 → none；缺 `expectedVersion` → 400；版本冲突 → 409；`batch` 原子写 |
| API | ingest / recompute complete：version 严格相等；错版本 409；旧 complete 不清新 stale |
...
- [ ] PUT 必填 `expectedVersion`；`env.DB.batch()` 原子写配置 / version / stale  
- [ ] 业务变更 `recomputeKind` 一期仅为 `full_rematch`（与 02 day_key 规则一致）  
- [ ] ingest / recompute complete：`pipelineConfigVersion` **严格相等**  

...
- Each policy's conclusion, required/enabled/applicable evidence, explicit expiry and independently reported build-currentness; review thresholds where available.
...
Compact evidence initially caused an active-build sample with a negative deferred PoP evaluation to receive Attention. The rubric now explicitly distinguishes deferred final steps from actions needed now; real reevaluation returned Running without a code override. Successful builds carrying both `buildIsNotCurrent:true` and `isExpired:false`, together with failed review compliance, still received Attention in this sample. Those mixed-evidence cases remain calibration examples, not asserted Review Needed or expiry ground truth. No confidence threshold or deterministic fallback conceals the model's choice.

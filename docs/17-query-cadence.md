...
> 状态：待 Review，尚未实现。总览见 [14](14-collector-architecture.md)，调度规则见 [16](16-scheduler-state-machine.md)，CLI 输出见 [18](18-cli-query-contract.md)。

## 1. 三种独立的周期

| 周期 | 谁决定 | 建议默认值 | 做什么 |
| --- | --- | --- | --- |
| 源站采集 | 观察刷新 Scheduler；外围 DiscoveryController | 项目观察轮次完成后 5 分钟；外围项目发现任务全部仓库结束后 2 分钟 | 刷新只覆盖 active 观察项；发现必须有明确任务，结果不自动加入观察 |
| 网页 Query | 各数据块 ViewModel | 前台 PR / 观察列表 / Collector 读取完成后 3 秒；项目仓库目录读取完成后 30 秒 | 读取 SignOff 已有数据，不触发 ADO，不改变观察列表 |
...
网页读取失败按 3、6、12、24、30 秒上限退避，恢复成功后回到正常读取间隔；后台不保留这些自动读取定时器。手动 Retry 和重新进入前台可以立即重试读取。

**提高消费者查询频率不会提高 ADO 采集频率。** 需要持续检查时显式加入观察列表；需要提前更新时对观察项发 refresh；需要发现仓库 PR 时发 discover。三类动作分开，均不能藏在 GET 里。

## 2. 时间线示例

假设外围控制器向某项目提交的 list 花了 35 秒，该项目观察项的 checks 轮次花了 80 秒：

```text
10:00:00  list 开始
10:00:35  list 全部目标结束并发布
10:02:35  外围控制器最早提交下一次 list（完成 + 120 秒冷却）

10:00:10  checks 开始
10:01:30  checks 全部目标结束
10:06:30  下一轮 checks 最早开始（80 秒完成 + 300 秒冷却）

10:01:40  外部 CLI 查询 → 立即取得已有快照
10:02:40  外部 CLI 查询 → 仍可能看到进行中轮次之前的快照
```

同项目的 list 与 checks 还受单个运行任务的约束；上述是各自冷却的计算示例，不承诺两个任务同时执行。队列等待、限流和任务耗时会让实际开始更晚。
...
当前归一化器以采集任务传入的时间生成 `observedAt` 等字段。迁移时保持旧记录的原始时间；新采集应记录成功取得事实的时间，不能在查询、展示或重试失败时补成 `now`。原始检查被 list 复用时，`checksObservedAt` 保持不变。
...
- 慢任务：list 与 checks 均按最后完成时间加冷却启动，不按浏览器计时、不补偿式连跑。

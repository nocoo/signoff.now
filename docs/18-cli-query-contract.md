...
| `readiness` | One last business `state`, its source/model/rubric/fingerprint and evaluation/reuse times, `isCurrent`, and collection/evaluation `phase` with operational `update` details. No duplicated labels, previous/current objects, model probabilities or rule-generated readiness. |
...
`readiness.state` is `conflict`, `skipped`, `attention`, `review_needed`, `warning`, `running`, `ready`, `waiting`, or null. Non-main targets are directly Skipped; active provider conflicts are directly Conflict; other categories use the saved Jev Choice. Ready retains the configured final PoP convention and cannot override provider merge requirements.

`isCurrent` means the saved judgment matches **cached decision evidence and current instructions/configuration**. The read-only projection compares the same canonical decision evidence used by the scheduler, including collection-validity changes, but never calls Jev. A ten-minute-old collection can still be consistent; consistency is not proof of source freshness. `generatedAt` is query time, not collection time.

Changed evidence retains the last state with `isCurrent:false`. `update.state` distinguishes `idle`, `scheduled`, `evaluating`, `blocked`, `error`, and `stopped`; `reason` distinguishes awaiting collection and unwatched. `notBefore` is the earliest eligibility time from retry/per-PR cooldown, **not a promised execution time**. The persistent collector daemon starts eligible Jev work even when the dashboard is closed. Query requests never initiate it. Missing credentials or failed inference appear in `update.error`, separately from a failed CI check.
...
- Advice codes are `resolve_conflict`, `inspect_pr`, `observe`, `wait_ci`, `verify_merge`, `request_review`, `none`. Attention only advises inspection; it does not decide repair versus CI rerun. A stale judgment's advice remains historical and must be considered together with `isCurrent`. Before writing to ADO, the consumer verifies relevant facts live under its own authorization.
...
The expanded sidebar displays a stacked one-hour network chart above Connector. It uses 60 one-minute buckets (the current minute is partial), three blue ADO series and a purple Jev series. Hover shows each series count for that minute. The chart reads `GET /api/query/v1/network` every 10 seconds while the document is visible, pauses when hidden, and refreshes immediately on visibility return. Collapsing the sidebar unmounts the chart. These reads never schedule collection or inference and do not change provider cooldowns.

Counts represent actual HTTP attempts across all projects, not jobs or PRs. Discovery includes PR lists, repository metadata and authentication probes; PR details includes individual PRs, reviews, threads and changes; checks includes policies, statuses, builds and timelines. Jev includes independent PR requests and explicit connection tests. Pagination, retries, HTTP errors and transport failures count individually. Requests rejected before dispatch do not count. Browser-to-local-Worker traffic and Azure CLI's internal token-acquisition traffic are outside this provider chart.

Only a random attempt ID, category and start timestamp are persisted in `network_requests`; no URL, payload, response, PR identity or credentials are stored. Attempts appear after the fetch settles, in their start minute. Collector reports use the local-only validated `POST /api/collector/network` endpoint, with stable attempt IDs for idempotent retries, a two-second reporting timeout and bounded retries. Telemetry errors warn without changing provider results. A Worker outage can leave gaps; the chart is operational telemetry, not a billing ledger. Writes prune events older than two hours; reads expose only the rolling hour. History starts when instrumentation is enabled; earlier activity is not reconstructed from job counts. Counts persist across reloads and process restarts and are global even when the page shows sample data.

Validation covers retry counting, reporting failure isolation, deduplication, cutoff/zero buckets, input validation, local authorization and background polling pause/resume. Browser E2E checks persisted counts, four distinct stacked series, tooltip, reload and compact viewport layout. Live local checks verify real collector activity; deterministic fixtures verify the full color stack without spending Jev credits.

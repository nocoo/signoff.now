...
>
> **Naming note:** This document reflects the **current code** naming (e.g., `PrsReport`, `PrDetail`, `state: "open" | "closed"`). The target naming spec (aligned with GitHub GraphQL) is defined in [04-pulse-cli-spec.md](../cli-history/04-pulse-cli-spec.md). Type names, field names, and DB `state` column values will be migrated in the Rename phase described there.

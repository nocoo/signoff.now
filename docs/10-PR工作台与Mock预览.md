...
- 初版 SQL migration：`packages/db/migrations/0011_pr_workbench.sql`；当前 schema version 13，新增真实采集任务与当前页 PR 范围。
...
| Commerce | #2168 | 构建与评审均完成，只剩黄色 PoP 人工认证；描述包含 Markdown 表格与任务清单 |
...
预置项目可以模拟扫描，让各 build 的一个可运行阶段从 queued → running → passed，并恢复不完整的示例 timeline。草稿与终态 PR 不推进；失败、冲突、待评审、PoP 和环境审批继续保留。当前新增项目初始为空，第一次扫描使用真实 ADO 采集。

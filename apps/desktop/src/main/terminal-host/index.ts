 ...
 *
 * A persistent background process that owns PTYs and terminal state.
 ...
 * - Socket: ~/.signoff/terminal-host.sock
 * - Auth token: ~/.signoff/terminal-host.token
 */
...
import { SIGNOFF_DIR_NAME } from "shared/constants";
import {
...
const SIGNOFF_HOME_DIR = join(homedir(), SIGNOFF_DIR_NAME);
...
const PID_PATH = join(SIGNOFF_HOME_DIR, "terminal-host.pid");
...
async function startServer(): Promise<void> {
	if (!existsSync(SIGNOFF_HOME_DIR)) {
		mkdirSync(SIGNOFF_HOME_DIR, { recursive: true, mode: 0o700 });
		log("info", `Created directory: ${SIGNOFF_HOME_DIR}`);
	}

	try {
		chmodSync(SIGNOFF_HOME_DIR, 0o700);
	} catch {
...
async function main() {
	...
	log("info", `Home directory: ${SIGNOFF_HOME_DIR}`);

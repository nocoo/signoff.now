		...
		>
			...
			<div className="flex items-start gap-2.5">
				...
				<div className="min-w-0 flex-1">
					...
					<p className="mt-1 line-clamp-2 text-[11px] leading-4 text-basalt-muted-foreground">
						{data.detail}
					</p>
				</div>
				{data.active ? (
...
type Graph = MachineGraph;

async function arrange(graph: Graph) {
	const { default: ELK } = await import("elkjs/lib/elk.bundled.js");
	const stages = ["lifecycle", "gate", "aggregate", "state"];
	...
	return (
		...
		>
			...
			{!!arranging && (
				<p
					role="status"
					className="absolute left-4 top-16 z-10 rounded-lg bg-basalt-card px-3 py-2 text-xs"
				>
					Arranging graph…
				</p>
			)}
			...
			{mode === "observed" && !graph.edges.length ? (

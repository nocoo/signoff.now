...
export default function PolicyInstructionsPage() {
	const workbench = useWorkbench();
	const projects = workbench.data?.projects ?? [];
	const location = useLocation(),
		navigate = useNavigate();
	const route = parseWorkspaceLocation(
		location.pathname.replace(/^\/policy-instructions/, "/sm"),
	);
			...
			<div className="flex flex-wrap items-center gap-3">
				...
				<SelectControl
					...
					onChange={(v) => {
						...
						if (next) navigate(policyHref(next));
					}}
					className="w-72"
				>
					{projects.map((p) => (
			...
			{projectId ? (
				<PolicyEditor
					...
					onRepository={(id) => {
						...
						if (project) navigate(policyHref(project, next ?? null));
					}}
				/>
			) : (

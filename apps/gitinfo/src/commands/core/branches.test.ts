...
describe("getLocalBranches", () => {
	it("parses branches with tracking info", async () => {
		const exec = mockExec({
			"git for-each-ref --format=%(refname:short)%(09)%(upstream:short)%(09)%(upstream:trackshort)%(09)%(committerdate:iso-strict) refs/heads/":
				{
					stdout: [
						"main\torigin/main\t=\t2025-01-10T12:00:00+00:00",
						"feature\torigin/feature\t>\t2025-01-15T09:30:00+00:00",
		...
		expect(branches[1]?.aheadBehind).toEqual({ ahead: 1, behind: 0 });
	...
	it("handles behind tracking status", async () => {
		const exec = mockExec({
			"git for-each-ref --format=%(refname:short)%(09)%(upstream:short)%(09)%(upstream:trackshort)%(09)%(committerdate:iso-strict) refs/heads/":
				{
					stdout: "stale\torigin/stale\t<\t2025-01-05T08:00:00+00:00",
				},
			"git branch --merged HEAD": { stdout: "" },
		});

		const branches = await getLocalBranches(exec, CWD, true);
		expect(branches[0]?.aheadBehind).toEqual({ ahead: 0, behind: 1 });
	});
	...
	it("handles diverged tracking status", async () => {
		const exec = mockExec({
			"git for-each-ref --format=%(refname:short)%(09)%(upstream:short)%(09)%(upstream:trackshort)%(09)%(committerdate:iso-strict) refs/heads/":
				{
					stdout: "diverged\torigin/diverged\t<>\t2025-01-05T08:00:00+00:00",
				},
			"git branch --merged HEAD": { stdout: "" },
		});

		const branches = await getLocalBranches(exec, CWD, true);
		expect(branches[0]?.aheadBehind).toEqual({ ahead: 1, behind: 1 });
	});
});
